...
# Serve static files (admin dashboard, chat widget, etc.)
...
class TaskManager:
    ...
    def get_task(self, task_id: str, user_id: str) -> Optional[Dict]:
        """Get a single task by ID, verifying user access"""
        try:
    ...
    def update_task(self, task_id: str, user_id: str, update_data: dict) -> bool:
        ...
        try:
            ...
            safe_fields = {"title", "priority", "urgency", "complexity", "location",
                          "experience", "skills", "status", "description", "comment", "feedback"}
    ...
    def assign_task(self, user_id: str, task_id: str, assignee_id: str) -> bool:
        """Assign task to another user (only creator can assign)"""
        # Verify ownership
        ...
        result = db.tasks.update_one(
            {"_id": ObjectId(task_id)},
            {"$set": {"assigned_to": assignee_id, "updated_at": datetime.utcnow()}}
        )
        return result.modified_count > 0
...
"""
    ...
    def send_email(self, content: str, recipient: str = None, subject: str = None, recruiter_name: str = None) -> bool:
        """Send EOD email safely with personalized subject"""
        ...
        if not subject:
            if recruiter_name:
                subject = f"🎯 Recruiter OS - EOD Summary for {recruiter_name} ({datetime.now().strftime('%d %b %Y')})"
            else:
                subject = f"🎯 Recruiter OS - Daily EOD Summary ({datetime.now().strftime('%d %b %Y')})"
...
class AdminAuthManager:
    ...
    def authenticate_admin(username: str, password: str) -> Optional[Dict]:
        ...
        try:
            if not verify_password(password.strip(), password_hash):
                print(f"❌ Admin Auth Failed: Password mismatch for '{username}'")
                return None
        except Exception as e:
...
# Function to ensure default users exist and have correct credentials
def ensure_default_users():
    try:
        print("🔄 Verifying default user credentials...")
        
        # 1. Ensure Admin User (admin / admin123)
        admin_data = {
            "username": "admin",
        ...
        print("✅ Default Admin OK: admin / admin123")
        
        # 2. Ensure Recruiter User (recruiter / recruiter123)
        recruiter_data = {
            "username": "recruiter",
        ...
        db.users.update_one(
            {"username": "recruiter"},
            {"$set": recruiter_data, "$setOnInsert": {"created_at": datetime.utcnow()}},
            upsert=True
        )
        print("✅ Default Recruiter OK: recruiter / recruiter123")

    except Exception as e:
        print(f"⚠️ Could not ensure default users: {e}")
...
def _is_valid_email(email: str) -> bool:
    """Check if email is a valid, real email (not dummy/placeholder)"""
    if not email:
        return False
    dummy_patterns = ["@example.com", "@hrms.com", "@test.com", "@localhost"]
...
def daily_eod_job():
    ...
    Fetches each recruiter's real email from MongoDB and sends personalized EOD."""
    try:
        # Find all recruiters from MongoDB
        recruiters = list(db.users.find({"role": "recruiter"}))
        if not recruiters:
            # Fallback: find all users if no 'recruiter' role is set
            recruiters = list(db.users.find())
        ...
        print(f"\n{'=' * 60}")
        print(f"🚀 EOD SCHEDULER TRIGGERED - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"   Found {total} recruiters in MongoDB")
        print(f"{'=' * 60}")
        ...
        for user in recruiters:
            ...
            # Skip recruiters without a valid real email
            if not _is_valid_email(user_email):
                print(f"  ⏭️  Skipping '{username}' — no valid email (has: '{user_email or 'None'}')")
                skipped_count += 1
                admin_summary_lines.append(f"⏭️ {username}: Skipped (no valid email)")
                continue

            print(f"  📧 Generating EOD for '{username}' → {user_email}")

            try:
                # Generate comprehensive EOD summary
                summary = master_agent.generate_comprehensive_eod(user_id)

                # Send personalized email to the recruiter's real email from MongoDB
                email_sent = agents.daily_agent.send_email(
                    content=summary,
                ...
                if email_sent:
                    sent_count += 1
                    admin_summary_lines.append(f"✅ {username}: Sent to {user_email}")
                    print(f"    ✅ EOD sent to {username} ({user_email})")
                else:
                    failed_count += 1
                    admin_summary_lines.append(f"❌ {username}: Email send failed")
                    print(f"    ❌ EOD email failed for {username}")

                # Log EOD send to MongoDB for tracking
                db.eod_logs.insert_one({
                    "user_id": user_id,
                    "username": username,
                    "email": user_email,
                    "sent_at": datetime.utcnow(),
                ...
                failed_count += 1
                admin_summary_lines.append(f"❌ {username}: Error - {str(inner_e)[:50]}")
                print(f"    ❌ Error for {username}: {inner_e}")

        # --- Send Admin Summary ---
        admin_report = f"""
🎯 RECRUITER OS - ADMIN EOD DISPATCH SUMMARY
{'=' * 55}
📅 Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
👥 Total Recruiters: {total}
✅ Emails Sent: {sent_count}
⏭️ Skipped (no email): {skipped_count}
❌ Failed: {failed_count}

📋 DETAILED STATUS:
{'=' * 55}
"""
        for line in admin_summary_lines:
            admin_report += f"  {line}\n"

        admin_report += f"\n{'=' * 55}\n🤖 This is an automated report from Recruiter OS EOD Scheduler.\n"

        # Send summary to admin (EOD_RECEIVER from .env)
        try:
            agents.daily_agent.send_email(
                content=admin_report,
                recipient=EOD_RECEIVER,
                subject=f"📊 Admin EOD Dispatch Summary — {sent_count}/{total} sent ({datetime.now().strftime('%d %b %Y')})"
            )
            print(f"  📊 Admin summary sent to {EOD_RECEIVER}")
        except Exception as admin_e:
            print(f"  ⚠️ Failed to send admin summary: {admin_e}")

        print(f"\n{'=' * 60}")
        print(f"✅ EOD JOB COMPLETE — Sent: {sent_count} | Skipped: {skipped_count} | Failed: {failed_count}")
        print(f"{'=' * 60}\n")
...
print("✅ Daily EOD Scheduler Started (7 PM IST — sends to each recruiter's email from MongoDB)")
...
# Login endpoint for recruiter users
@app.post("/api/login")
def login(req: LoginRequest):
    ...
    if not user:
        ...
        # Use 400 to bypass frontend 401 interceptor and show specific error
        raise HTTPException(
            status_code=400, 
            detail=f"User '{username}' does not exist. Please contact admin."
        )
    
    # Get password hash (fallback to legacy 'password' key if 'password_hash' is missing)
    password_hash = user.get("password_hash") or user.get("password")
    
    # Check if password hash exists
    if not password_hash:
        print(f"❌ Login failed: No password hash found for user '{username}'")
        raise HTTPException(
            status_code=400,
            detail="Account configuration error: No password set. Contact admin."
        )
    
    # Verify password
    try:
        if not verify_password(password, password_hash):
            print(f"[FAIL] Login failed: Invalid password for user '{username}'")
            raise HTTPException(
                status_code=400,
        ...
        print(f"[ERROR] Login failed: Password verification error for user '{username}': {e}")
        raise HTTPException(
            status_code=400,
            detail="Internal error verifying password. Contact admin."
        )
...
def debug_login():
    """Debug endpoint to check recruiter user status"""
    try:
        user = db.users.find_one({"username": "recruiter"})
        if not user:
            return {"status": "error", "message": "User 'recruiter' NOT FOUND in database"}
        
        # Check password fields
        has_hash = "password_hash" in user
        has_pass = "password" in user
        
        # Get hash for verification test
        stored_hash = user.get("password_hash") or user.get("password")
        
        # Test verification with "recruiter123"
        is_valid = False
        error_msg = None
        try:
            if stored_hash:
                is_valid = verify_password("recruiter123", stored_hash)
            else:
                error_msg = "No password hash stored"
        except Exception as e:
            error_msg = str(e)
        ...
        return {
            ...
            "has_legacy_password": has_pass,
            "stored_hash_start": stored_hash[:10] if stored_hash else None,
...
def assign_task(
    ...
    try:
        success = agents.task_manager.assign_task(current_user["id"], task_id, assignee_id)
        if not success:
            raise HTTPException(404, "Task not found or access denied")
...
def update_candidate_status(
    ...
    try:
        ...
        valid_statuses = ["applied", "shortlisted", "rejected", "hired", "interview scheduled"]
        ...
        if result.modified_count > 0:
            # Auto-update task status if shortlisted or interview scheduled
...
def manual_eod(current_user=Depends(get_current_user)):
    ...
    try:
        ...
        # Use recruiter's real email from MongoDB, fallback to admin EOD_RECEIVER
        if not _is_valid_email(user_email):
...
# ============================================================
# ROUTES - CANDIDATE REGISTRATION (OTP)
# ============================================================
...
async def register_candidate(req: CandidateRegisterRequest):
    """Candidate registration/login via OTP"""
    if req.action == "send_otp":
        ...
        # Send OTP via email (using existing SMTP logic)
        email_content = f"Your JobSetu verification code is: {otp}"
        email_sent = agents.daily_agent.send_email(email_content, req.email)
        
        # For development, if email fails, return true
        if not email_sent:
            print(f"⚠️ OTP for {req.email} is {otp} (email not sent)")
        ...
        stored = db.otps.find_one({"email": req.email})
        if not stored or stored.get("otp") != req.otp:
            return {"success": False, "message": "Invalid or expired OTP"}
            
        # Check if candidate exists
        candidate = db.candidates.find_one({"email": req.email})
        if not candidate:
            # Create a shell candidate profile
            candidate_doc = {
        ...
        # Generate token
        token = create_access_token(candidate_id)
        ...
        return {
            ...
            "data": {
                "token": token,
                "email": req.email,
                "name": candidate.get("name", req.email.split('@')[0]) if candidate else req.email.split('@')[0]
            }
        }
...
async def chatbot_route(req: ChatRequest):
    ...
    try:
        ...
        prompt = f"System: You are JobSetu AI Assistant specializing in {req.domain}. "
        prompt += "Context: You are helping a recruiter or candidate. "
        
        if req.history:
            prompt += "\nHistory:\n"
            for h in req.history[-5:]: # Last 5 messages
        ...
        if not response_text:
            return {"response": "I'm sorry, I'm having trouble connecting to my brain right now.", "sources": []}
            
        return {
            "response": response_text,
            "sources": ["JobSetu Database", f"{req.domain} Knowledge Base"]
        }
    except Exception as e:
...
def admin_login(request: AdminLoginRequest):
    ...
    print(f"🔐 Admin Login Attempt: {request.username}")
    admin = AdminAuthManager.authenticate_admin(request.username, request.password)
    
    if not admin:
        print(f"❌ Admin Login Failed for '{request.username}'")
        # Use 400 to show error in frontend
...
def get_admin_logs_alias(current_user=Depends(get_current_user)):
    """Alias for login-logs as expected by frontend"""
    return get_login_logs(current_user)
