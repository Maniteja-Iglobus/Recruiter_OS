...
def render_admin_chatbot():
    ...
    <div id="chatbot-container" style="
        ...
        z-index: 9999;
    ">
        <div id="chat-area" style="flex:1; padding:10px; overflow-y:auto;"></div>
        <input id="chat-input" placeholder="Ask Admin Assistant..."
            style="border:none; border-top:1px solid #ddd; padding:12px; outline:none;" />
    </div>

    <script>
    const API_URL = "https://reatha-thioacetic-sharla.ngrok-free.dev/chat";
    let history = [];
    ...
    document.getElementById("chat-input").addEventListener("keypress", async function(e) {
        if (e.key === "Enter" && this.value.trim()) {
            ...
            try {
                ...
                chatArea.innerHTML += `<div><b>Bot:</b> ${data.response}</div>`;
                ...
                chatArea.innerHTML += `<div style="color:red;">Error connecting to chatbot</div>`;
            }
        }
    });
    </script>
...
def show_admin_dashboard():
    ...
    def render_admin_chatbot():
        ...
        <div style="
            ...
            <div id="chat-area" style="flex:1; padding:10px; overflow-y:auto;"></div>
            <input id="chat-input" placeholder="Ask Admin AI..."
        ...
        document.getElementById("chat-input").addEventListener("keypress", async function(e) {
            if (e.key === "Enter" && this.value.trim()) {
                ...
                try {
                    ...
                    const data = await res.json();
                    area.innerHTML += `<div><b>Bot:</b> ${data.response}</div>`;
                    ...
                    area.innerHTML += `<div style='color:red'>Connection error</div>`;
                }
            }
        });
        </script>
